...
namespace SonicHesap.Entities.Data_Access
{
    public class KasaDAL : EntityRepositoryBase<SonicHesapContext, Kasa,KasaValidator>
    {
        ...
        public object OdemeTuruToplamListele(SonicHesapContext context,int kasaId)
        {
            var result = (from c in context.KasaHareketleri.Where(c => c.KasaId == kasaId)
                          ...
                          select new
                          {
                              ...
                              Bakiye= (grp.Where(c => c.OdemeTuruId == grp.Key.OdemeTuru.Id && c.Hareket == "Kasa Giriş").Sum(c => c.Tutar) ?? 0) - (grp.Where(c => c.OdemeTuruId == grp.Key.OdemeTuru.Id && c.Hareket == "Kasa Çıkış").Sum(c => c.Tutar) ?? 0)
                          });
            return result;
        }

        public object GenelToplamListele(SonicHesapContext context, int kasaId)
        {
